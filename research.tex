...
\begin{enumerate}
\item Review existing literature on the most important side-channel attacks in the browser. (WP1 A)
\item Provide implementations for the selected browser attacks, optimised to work on the most recent versions of browser software. (WP1 B)
\item Provide new attack vectors for estimating the size of a cross-origin resource. (WP1 C)
\end{enumerate}
